...
function getState(key: string) {
  ...
  return localStorage.getItem(key) && JSON.parse(localStorage.getItem(key)).compare
    ? JSON.parse(localStorage.getItem(key)).compare
    : initialState;
}

export function compareReducer(state = getState('vio'), action) {
  switch (action.type) {
    case ADD_TO_COMPARE:
